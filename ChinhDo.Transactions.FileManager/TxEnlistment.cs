...
namespace ChinhDo.Transactions
{
  public partial class TxFileManager
  {
    ...
    private class TxEnlistment : IEnlistmentNotification, IFileOperations
    {
      ...
      /// </summary>
      public bool IgnoreExceptionsInRollback { private get; set; }
      ...
      public void CreateDirectory(string path)
      {
        if (_tx != null)
        {
          ...
          string strNormalizedPath = m_rgxCleanPath.Replace(path,
          ...
          Int32 i = 0;
          string strPath = "";
          if (strPaths[0].EndsWith(Path.VolumeSeparatorChar.ToString(CultureInfo.InvariantCulture)))
          {
      ...
      {
        foreach (RollbackOperation t in _journal)
        {
          t.CleanUp();
        }
      ...
      private abstract class RollbackOperation
      {
        ...
        protected static string MakeTempFileName(string ext)
        {
          string retVal = null;
          do
          {
            retVal =
              Path.Combine(_tempFolder, _tempFilesPrefix + Path.GetFileNameWithoutExtension(Path.GetRandomFileName())) +
              ext;
          }
      ...
      private class RollbackFile : RollbackOperation
      {
        public RollbackFile(string fileName)
        {
          ...
          {
            _backupFileName = MakeTempFileName(Path.GetExtension(fileName));
