...
namespace ChinhDo.Transactions
{
  ...
  public partial class TxFileManager : IFileManager
  {
    ...
    /// <summary>
    /// Uses GetRandomFileName() in a loop to find a new temp file and returns the name.
    /// </summary>
    ...
    protected static string CreateTempFileName(string ext)
    {
      ...
      {
        retVal =
          Path.Combine(_tempFolder, _tempFilesPrefix + Path.GetFileNameWithoutExtension(Path.GetRandomFileName())) + ext;
      }
      while (File.Exists(retVal));
    ...
    private const string _tempFilesPrefix = "";
