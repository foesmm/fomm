...
namespace Fomm.Games.Fallout3.Tools.TESsnip
{
  ...
  internal struct SubrecordStructure
  {
    ...
    public SubrecordStructure(XmlNode node)
    {
      ...
      List<ElementStructure> elementStructures = new List<ElementStructure>();
      foreach (XmlNode n in node.ChildNodes)
      {
        ...
        elementStructures.Add(new ElementStructure(n));
      }
      elements = elementStructures.ToArray();
      ContaintsConditionals = false;
      bool containsBlob = false;
      foreach (ElementStructure es in elements)
      {
        if (es.CondID != 0)
        {
  ...
  internal class RecordStructure
  {
    ...
    private RecordStructure(XmlNode node, Dictionary<uint, SubrecordStructure[]> groups)
    {
      ...
      List<SubrecordStructure> subrecordStructures = new List<SubrecordStructure>();
      ...
      subrecords = subrecordStructures.ToArray();
      ...
      for (int i = 0; i < subrecords.Length; i++)
      {
        ...
        int j = Math.Max(subrecords[i].optional, subrecords[i].repeat) + i - 1;
