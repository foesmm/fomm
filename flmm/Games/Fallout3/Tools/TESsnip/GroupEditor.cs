...
namespace Fomm.Games.Fallout3.Tools.TESsnip
{
  internal partial class GroupEditor : Form
  {
    ...
    public GroupEditor(GroupRecord gr)
    {
      ...
      tbX.Text = TypeConverter.h2ss(data[2], data[3]).ToString(CultureInfo.InvariantCulture);
      tbY.Text = TypeConverter.h2ss(data[0], data[1]).ToString(CultureInfo.InvariantCulture);
      tbBlock.Text = TypeConverter.h2i(data[0], data[1], data[2], data[3]).ToString(CultureInfo.InvariantCulture);
    ...
    {
      if (tbRecType.Text.Length < 4)
      {
        tbRecType.Text = tbRecType.Text.PadRight(4, '_');
      }
    }
  }
}