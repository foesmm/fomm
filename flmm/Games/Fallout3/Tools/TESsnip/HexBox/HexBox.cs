...
namespace Fomm.Games.Fallout3.Tools.TESsnip.HexBox
{
  ...
  internal class HexBox : Control
  {
    ...
    private void UpdateScrollSize()
    {
      ...
      {
        var scrollmax =
          (long) Math.Ceiling(_byteProvider.Length/(double) HorizontalByteCount - VerticalByteCount);
    ...
    public void ScrollByteIntoView(long index)
    {
      ...
      if (index < _startByte)
      {
        var line = (long) Math.Floor((double) index/HorizontalByteCount);
      ...
      switch (_borderStyle)
      {
        case BorderStyle.Fixed3D:
        {
          if (TextBoxRenderer.IsSupported)
          {
            ...
            var rectContent = vsr.GetBackgroundContentRectangle(pevent.Graphics, ClientRectangle);
    ...
    private void PaintHex(Graphics g, long startByte, long endByte)
    {
      ...
      var counter = -1;
      var intern_endByte = Math.Min(_byteProvider.Length - 1, endByte + HorizontalByteCount);
    ...
    private void PaintHexStringSelected(Graphics g, byte b, Brush brush, Brush brushBack, Point gridPoint)
    {
      ...
      var isLastLineChar = (gridPoint.X + 1 == HorizontalByteCount);
    ...
    {
      var currentLine = (long) Math.Floor((double) _bytePos/HorizontalByteCount) + 1;
