...
namespace Fomm.Games.Fallout3.Tools.TESsnip.ScriptCompiler
{
  internal static class ScriptCompiler
  {
    ...
    private class FunctionSig
    {
      ...
      private static VarType GetVarType(string s)
      {
        switch (s)
        {
          ...
          default:
            throw new RecordXmlException("Invalid variable type: " + s);
        }
      }
      ...
      public FunctionSig(XmlNode n, bool block)
      {
        ...
        if (!block)
        {
          ...
          if (n2 != null)
          {
            paddingbytes = byte.Parse(n2.Value);
          }
        }
    ...
    private static void RecursePlugin(Rec rec, uint mask, uint id, Dictionary<uint, Record> records,
                                      List<Pair<uint, Record>> quests, List<Pair<uint, Record>> refs)
    {
      if (rec is Record)
      {
        Record r2 = (Record) rec;
      ...
      {
        foreach (Rec r2 in ((GroupRecord) rec).Records)
        {
    ...
    private static void RecursePlugin(Rec rec, Dictionary<uint, Record> records, List<Pair<uint, Record>> quests,
    ...
    public static void Setup(Plugin[] plugins)
    {
      ...
      for (uint i = 0; i < plugins.Length - 1; i++)
      {
        ...
        if (plugins[i].Records.Count == 0 || plugins[i].Records[0].Name != "TES4")
        {
          continue;
        }
        mask = 0;
        ...
        mask <<= 24;
        uint id = i << 24;
        foreach (Rec rec in plugins[i].Records)
        {
          RecursePlugin(rec, mask, id, records, quests, refs);
        }
        ...
        foreach (Pair<uint, Record> recs in refs)
        {
          ...
          {
            Record record = records[recs.Key];
            uint formID = 0;
      ...
      foreach (Rec rec in plugins[plugins.Length - 1].Records)
      {
        RecursePlugin(rec, records, quests, refs);
      }
      foreach (Pair<uint, Record> recs in refs)
      {
        if (RefLookupTable.ContainsKey(recs.Key) && RefLookupTable[recs.Key] != 0)
    ...
    private static void EmitExpression(IEnumerable<Token> smt, ExpressionType type)
    {
      long pos = bw.BaseStream.Length;
      Emit(0);
      try
