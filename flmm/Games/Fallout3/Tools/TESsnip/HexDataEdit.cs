...
namespace Fomm.Games.Fallout3.Tools.TESsnip
{
  internal partial class HexDataEdit : Form
  {
    ...
    private void hexBox1_SelectionStartChanged(object sender, EventArgs e)
    {
      ...
      {
        tbFloat.Text =
          TypeConverter.h2f(bytes[pos], bytes[pos + 1], bytes[pos + 2], bytes[pos + 3])
                       .ToString(CultureInfo.InvariantCulture);
        tbFloat.Enabled = true;
        tbInt.Text =
          TypeConverter.h2si(bytes[pos], bytes[pos + 1], bytes[pos + 2], bytes[pos + 3])
                       .ToString(CultureInfo.InvariantCulture);
        tbInt.Enabled = true;
      ...
      {
        tbWord.Text = TypeConverter.h2ss(bytes[pos], bytes[pos + 1]).ToString(CultureInfo.InvariantCulture);
        tbWord.Enabled = true;
        bCWord.Enabled = true;
      }
      else
