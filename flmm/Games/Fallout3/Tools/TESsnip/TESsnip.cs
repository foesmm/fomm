...
namespace Fomm.Games.Fallout3.Tools.TESsnip
{
  ...
  internal partial class TESsnip : Form
  {
    ...
    private void openNewPluginToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      {
        MessageBox.Show("Cannot modify contents while searching", Resources.ErrorStr);
        return;
      }
    ...
    private void saveToolStripMenuItem_Click(object sender, EventArgs e)
    {
      if (PluginTree.SelectedNode == null)
      {
        MessageBox.Show("No plugin selected to save", Resources.ErrorStr);
        return;
      }
    ...
    private void cutToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      {
        MessageBox.Show("Cannot cut a plugin", Resources.ErrorStr);
        return;
      }
    ...
    private void copyToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      {
        MessageBox.Show("Cannot copy a plugin", Resources.ErrorStr);
      }
      else
      {
    ...
    private void pasteToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      if (Clipboard == null)
      {
        MessageBox.Show("The clipboard is empty", Resources.ErrorStr);
        return;
      }
      BaseRecord node = (BaseRecord) PluginTree.SelectedNode.Tag;
    ...
    private void listView1_ItemActivate(object sender, EventArgs e)
    {
          ...
          !SubrecordStructs[listView1.SelectedIndices[0]].UseHexEditor)
      {
        ...
        {
          MessageBox.Show("Subrecord doesn't seem to conform to the expected structure.", Resources.ErrorStr);
          re = null;
        }
        if (re != null)
        {
    ...
    private void sanitizeToolStripMenuItem_Click(object sender, EventArgs e)
    {
      if (PluginTree.SelectedNode == null)
      {
        MessageBox.Show("No plugin selected", Resources.ErrorStr);
        return;
      }
      TreeNode tn = PluginTree.SelectedNode;
      while (!(tn.Tag is Plugin))
    ...
    private void dumpEDIDListToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      if (PluginTree.SelectedNode.Tag is Record)
      {
        MessageBox.Show("Spell works only on plugins or record groups", Resources.ErrorStr);
        return;
      }
    ...
    private void compileScriptToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      if (SelectedSubrecord && parentRecord.Name != "SCPT")
      {
        ...
        if (sr.Name != "SCTX")
        {
          MessageBox.Show("You need to select a SCPT record or SCTX subrecord to compile", Resources.ErrorStr);
          return;
        }
    ...
    private void generateLLXmlToolStripMenuItem_Click(object sender, EventArgs e)
    {
...

      {
        ...
        if (!firstwasfallout)
        {
          MessageBox.Show("Only works on plugin's whose first master is fallout3.esm", Resources.ErrorStr);
          return;
        }
      }
      ...
      while (recs.Count > 0)
      {
        ...
        if (rec is GroupRecord)
        {
          ...
          {
            foreach (Rec lrec in gr.Records)
            {
              recs.Enqueue(lrec);
            }
          }
        ...
        else
        {
          ...
          switch (r.Name)
          {
            case "LVLI":
              ...
              break;
            case "LVLN":
              foreach (SubRecord subrec in r.SubRecords)
              {
                if (subrec.Name == "LVLO")
                {
                  if (subrec.Size != 12)
    ...
    */
    ...
    private void FindMasters()
    {
      ...
      for (int i = 0; i < masters.Count; i++)
      {
        ...
        {
          if (masters[i] == plugin.Name.ToLowerInvariant())
          {
            FormIDLookup[i] = plugin;
            uint fixup = 0;
            if (plugin.Records.Count > 0 && plugin.Records[0].Name == "TES4")
            {
              foreach (SubRecord sr in ((Record) plugin.Records[0]).SubRecords)
              {
                if (sr.Name == "MAST")
                {
                  fixup++;
