...
namespace Fomm.SharpZipLib.Zip
{
  ...
  internal class ZipOutputStream : DeflaterOutputStream
  {
    ...
    /// <param name="ofs">The offset of the first byte to write.</param>
    ...
    public override void Write(byte[] buffer, int ofs, int count)
    {
      if (curEntry == null)
      {
        throw new InvalidOperationException("No open entry.");
      ...
      if (ofs < 0)
      {
        throw new ArgumentOutOfRangeException("ofs", "Cannot be negative");
      }
      ...
      if ((buffer.Length - ofs) < count)
      {
        throw new ArgumentException("Invalid offset/count combination");
      }

      crc.Update(buffer, ofs, count);
      size += count;

      switch (curMethod)
      {
        case CompressionMethod.Deflated:
          base.Write(buffer, ofs, count);
          break;

        case CompressionMethod.Stored:
          baseOutputStream_.Write(buffer, ofs, count);
          break;
      }
    }
