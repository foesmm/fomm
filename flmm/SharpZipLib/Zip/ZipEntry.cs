...
namespace Fomm.SharpZipLib.Zip
{
  ...
  internal class ZipEntry : ICloneable
  {
    ...
    /// </returns>
    /// <see cref="Fomm.SharpZipLib.Zip.CompressionMethod.Deflated"/>
    /// <see cref="Fomm.SharpZipLib.Zip.CompressionMethod.Stored"/>
    ...
    internal void ProcessExtraData(bool localHeader)
    {
      ...
      else if (extraData.Find(0x5455))
      {
        ...
        int readFlags = extraData.ReadByte();

        // Can include other times but these are ignored.  Length of data should
        // actually be 1 + 4 * no of bits in flags.
        if (((readFlags & 1) != 0) && (length >= 5))
        {
