...
namespace Fomm.SharpZipLib.Zip.Compression
{
  ...
  internal class Deflater
  {
    ...
    */
    ...
    /// </param>
    public void SetLevel(int lLevel)
    {
      if (lLevel == DEFAULT_COMPRESSION)
      {
        lLevel = 6;
      }
      else if (lLevel < NO_COMPRESSION || lLevel > BEST_COMPRESSION)
      {
        throw new ArgumentOutOfRangeException("lLevel");
      }

      if (level != lLevel)
      {
        level = lLevel;
        engine.SetLevel(lLevel);
      }
    }
