...
*/
    ...
    {
      return copyBuffer_ ?? (copyBuffer_ = new byte[bufferSize_]);
    }
 ...
 */
    ...
    private void AddEntry(ZipFile workFile, ZipUpdate update)
    {
      ...
      {
        source = update.GetSource() ?? updateDataSource_.GetSource(update.Entry, null);
      }
    ...
    private class ZipUpdate
    {
      ...
      public ZipEntry OutEntry
      {
        get
        {
          return outEntry_ ?? (outEntry_ = (ZipEntry) entry_.Clone());
        }
      }
