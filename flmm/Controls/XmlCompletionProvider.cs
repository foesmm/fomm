...
namespace Fomm.Controls
{
  ...
  {
    private readonly List<XmlCompletionData> m_lstAutoCompleteList;
    private readonly string m_strElementPath;
    private readonly string[] m_strSiblings;
    private readonly AutoCompleteType m_actType = AutoCompleteType.Element;
    private readonly string m_strLastWord;
    private readonly List<char> m_lstExtraInsertionCharacters = new List<char>();
  ...
  public class XmlCompletionProvider : ICompletionDataProvider
  {
    ...
    private readonly Regex rgxTagContents = new Regex("<([^!>][^>]*)>?", RegexOptions.Singleline);
    private readonly Regex rgxTagName = new Regex(@"([^!/\s][^/\s]*)");
    private readonly Regex rgxLastAttribute = new Regex(".*\\s([^=]+)=?", RegexOptions.Singleline);
    private readonly Regex rgxAttribute = new Regex(@"(\S+)=");

    private XmlSchemaSet m_xstSchema;
    private XmlSchema m_xshSchema;
    private readonly ImageList m_imlImages;
    private string m_strPreSelection;
    private AutoCompleteType m_actCompleteType = AutoCompleteType.Element;

    private readonly Dictionary<AutoCompleteType, List<char>> m_dicExtraCompletionCharacters =
    ...
    private IEnumerable<KeyValuePair<string, string>> GetAutoCompleteList(XmlSchemaElement p_xseElement,
                                                                          List<string> p_lstSiblings,
      ...
      if (GotAutoCompleteList != null)
      {
        ...
        if (ProcessKey(p_chrCharTyped) == CompletionDataProviderKeyResult.NormalKey)
        {
          ...
          if (booInsideValue)
          {
            ...
            {
              strLastWord = p_chrCharTyped.ToString(CultureInfo.InvariantCulture);
            }
          }
          else
          {
